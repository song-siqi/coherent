...
class Environment(gym.Env, GymObservable, Recreatable):
    ...
    def _load_agents(self):
        ...
        for i, agent_config in enumerate(self.agents_config):
            # Add a name for the robot if necessary
            # assert "name" in agent_config and agent_config["name"] in {"franka", "aliengo", "quadrotor"}
            
            if "franka" in agent_config["name"]:
                agent = FrankaAgent(
                    name = agent_config["name"],
                    prim_path = "/World/" + agent_config["name"],
                    position = agent_config["position"],
                    orientation = agent_config["rotation"],
                    obs_modalities = agent_config["obs_modalities"]
                )
            elif "aliengo" in agent_config["name"]:
                agent = AliengoAgent(
                    name = agent_config["name"],
            ...
            elif "quadrotor" in agent_config["name"]:
                ...
                assert False, f"Agent Class is not implemented"
            
            # Import the robot into the simulator
            og.sim.import_agent(agent)
        
        if len(self.agents_config):
            # og.sim.reset(soft=False)
            # Auto-initialize all robots
            og.sim.play()
            # agent.initialize(physics_sim_view=og.sim._physics_sim_view)
            self.scene.reset()
            self.scene.update_initial_state()
            
            # # print(self.scene._initial_state)
            # pass
            # # 不能stop，否则Articulation的handles_initialize将为False了
            og.sim.stop()
    ...
    def _load_action_space(self):
        ...
        # action_space = gym.spaces.Dict({robot.name: robot.action_space for robot in {**self.robots, **self.agents}})
    ...
    def step(self, action):
        ...
        for agent in self.agents:
            obs = {**obs, **{agent.name: agent.get_obs()}}
