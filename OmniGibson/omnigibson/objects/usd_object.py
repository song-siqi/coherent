...
class USDObject(StatefulObject):
    ...
    def _load(self):
        ...
        if self._encrypted:
            ...
            # tmpp = self._name.split('_')
            # save_path = os.path.join("/home/pjlab/workspace/OmniGibson/omnigibson/data/og_dataset/objects/", 
            #             self._name[:len(self._name)-2-len(tmpp[-1])-len(tmpp[-2])],
            #             tmpp[-2], 'usd', tmpp[-2]+'.usd')
            # # print(save_path)
            # # print(os.path.exists(os.path.dirname(save_path)))
            # assert os.path.exists(os.path.dirname(save_path)), '%s' % save_path is not existed
